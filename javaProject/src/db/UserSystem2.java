...
// 메소드 정리 버전
	...
	public static void newPw() throws SQLException {
		...
		if (loginIdf == true) {
			...
			int updated = db.stmt.executeUpdate(
					"UPDATE TBL_USER SET PASSWORD = '" + newPW + "' WHERE USERID = '" + UserSystem2.id + "'");
			if (updated > 0) {
	...
	public static void check() throws SQLException {
		...
		DBClass db = new DBClass();
		if (loginIdf == true) {
			String sql = "SELECT * FROM TBL_USER WHERE USERID ='" + UserSystem2.id + "'";
	...
	public static void tal() throws SQLException {
		boolean loginIdf = login();
		DBClass db = new DBClass();
		Scanner s = new Scanner(System.in);
		if (loginIdf == true) {
			...
			String tal = s.next();
			if (tal.equals("Y")) {
				int updated = db.stmt.executeUpdate("DELETE FROM TBL_USER WHERE USERID ='"+UserSystem2.id +"'" );
